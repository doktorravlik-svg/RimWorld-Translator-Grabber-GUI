...
- ✅ **Морфологічна обробка**: PyMorphy3 для склонення (UA/RU), spaCy для PL/DE/FR (опціонально)
...
## 📝 Історія змін (2026-05)

### v2.4 — Оптимізація коду та виправлення редактора

#### 🐛 Виправлення

- **scanner/mod_scanner.py**: Виправлено помилку відступів у `_detect_language_from_text`
- **gui/tabs/editor/editor_file_browser.py**: Додано відсутній метод `tr()` для перекладу

#### ⚡ Оптимізації

- **scanner/mod_scanner.py**: Оптимізовано 6 функцій до O(n)
  - `_detect_language_from_text`: Однопроходний підсік символів із frozenset lookups
  - `parse_about_xml`: Однопроходний парсинг XML
  - `analyze_languages`: Генераторні вирази
  - `_scan_defs_for_languages`: Додано обмеження розміру файла (10MB) та ранній вихід
  - `detect_mod_languages`: Модульний імпорт
- **utils/loadfolders_parser.py**: Оптимізовано 2 функції з множинним видаленням дублікатів
- Додано обмеження: розмір файла (10MB), довжина тексту (5000 символів)

### v2.3 — Автоматичне розбиття глосарію
