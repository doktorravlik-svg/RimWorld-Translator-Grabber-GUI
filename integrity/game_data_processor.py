...
class GameReferenceManager:
    def __init__(self, game_path: str, lang="Russian"):
        ...
        self.key_to_mod = {}  # ✅ НОВОЕ: ключ -> имя мода/DLC
    ...
    def _get_lang_folder_name(self, base_path: str) -> str | None:
        ...
        if lang in lang_display_names:
            display_name = lang_display_names[lang]
            lang_variants.append(f"{self.lang} ({display_name})")
            lang_variants.append(f"{display_name}.tar")
        ...
        for item in os.listdir(base_path):
            ...
            for key, variants in lang_folder_map.items():
                if lang == key:
                    for v in variants:
                        if v.lower() in item_lower:
    ...
    def load_all_official_data(self) -> bool:
        ...
        for dlc in OFFICIAL_DLC:
            ...
            if os.path.exists(dlc_lang_path):
                self._scan_languages_folder(dlc_lang_path, dlc)

        return len(self.reference_db) > 0

    def _scan_languages_folder(self, languages_path: str, dlc_name: str):
        """
        Сканирует папку Languages, обрабатывая как распакованные папки, так и .tar архивы.

        Args:
            languages_path: Путь к папке Languages
            dlc_name: Название мода/DLC (например, "Core", "Odyssey")
        """
        ...
        if lang_folder.endswith(".tar"):
            ...
            if temp_dir:
                try:
                    self._scan_dir(temp_dir, dlc_name)
                finally:
            ...
            # Обычная папка
            self._scan_dir(lang_folder, dlc_name)

    def _scan_dir(self, path: str, dlc_name: str):
        """Рекурсивно ищет XML и вытягивает строки."""
        for root, _, files in os.walk(path):
            for fn in files:
                if fn.endswith(".xml"):
                    self._parse_file(os.path.join(root, fn), dlc_name)

    def _parse_file(self, file_path: str, dlc_name: str):
        try:
