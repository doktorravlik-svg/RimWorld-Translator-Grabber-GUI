...
def enhance_def_element(elem: etree._Element) -> None:
    """
...
def _enhance_pawn_kind_def(elem: etree._Element) -> None:
...
def _enhance_thing_def(elem: etree._Element) -> None:
        ...
        for li in verbs:
            if li.tag in ("li",) or li.tag.isdigit():
                ...
                if verb_class is not None and li.find("label") is None:
                    label_elem = etree.SubElement(li, "label")
    ...
    if stuff_props is not None and stuff_props.find("stuffAdjective") is None:
        ...
        stuff_adjective = etree.SubElement(stuff_props, "stuffAdjective")
        stuff_adjective.text = label


def _enhance_faction_def(elem: etree._Element) -> None:
    """FactionDef: добавляет pawnSingular и leaderTitle."""
    if elem.find("pawnSingular") is None:
        pawn_singular = etree.SubElement(elem, "pawnSingular")
        pawn_singular.text = "member"

    if elem.find("leaderTitle") is None:
        leader_title = etree.SubElement(elem, "leaderTitle")
        leader_title.text = "leader"


def _enhance_vef_ability_def(elem: etree._Element) -> None:
    ...
    if elem.find("jobReportString") is None:
        job_report = etree.SubElement(elem, "jobReportString")
    ...
    if verb_props is None:
        verb_props = etree.SubElement(elem, "verbProperties")

    if verb_props.find("label") is None:
        label_elem = etree.SubElement(verb_props, "label")
        label_elem.text = label_text


def _enhance_backstory_def(elem: etree._Element) -> None:
    ...
    if elem.find("titleFemale") is None:
        title_female = etree.SubElement(elem, "titleFemale")
        title_female.text = title

    if elem.find("titleShortFemale") is None:
        title_short_female = etree.SubElement(elem, "titleShortFemale")
        title_short_female.text = title


def _enhance_trait_def(elem: etree._Element) -> None:
    ...
    for li in degree_datas:
        if li.find("labelFemale") is None:
            ...
            label_female = etree.SubElement(li, "labelFemale")
            label_female.text = label


def _enhance_quest_script_def(elem: etree._Element) -> None:
    """
...
def _replace_variables_in_element(elem: etree._Element, variables: dict) -> None:
