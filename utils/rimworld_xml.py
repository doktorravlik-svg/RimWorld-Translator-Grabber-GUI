...
# Полный список переводимых тегов RimWorld (объединённый из RimTrans + Text-grabber)
TRANSLATABLE_TAGS = {
    # Базовые теги
    "label",
    "labelShort",
...
def extract_subfields(
    ...
    for child in element:
        ...
        if tag == "li":
            ...
            cur_pre = f"{prefix}.{tag}" if prefix else tag
        ...
        # ИЛИ частично совпадать (Part_of_tag_to_extraction из Text-grabber)
        tag_matches_whitelist = tag in whitelist_tags
        if not tag_matches_whitelist and whitelist_tags and partial_tag_matches:
            # Частичное совпадение - ищем если тег содержит один из partial_tag_matches
        ...
        if whitelist_tags and not tag_matches_whitelist:
            # Fallback по пробелу (из Text-grabber)
            # Если текст содержит пробел и длиннее min_text_length - извлекаем
            if enable_space_fallback and child.text and child.text.strip() and " " in child.text:
                ...
                if len(text) >= min_text_length and len(text) < max_text_length:
                    # Это вероятно переводимый текст даже если тег не в whitelist
                    fields[cur_pre] = text

            # Рекурсивно обрабатываем детей даже если тег не в whitelist
            if len(child) > 0:
                fields.update(
                    extract_subfields(
                        child,
        ...
        # ✅ Проверяем blacklist_patterns — ключ не должен содержать паттернов
        if blacklist_patterns:
            key_lower = cur_pre.lower()
            if any(pattern.lower() in key_lower for pattern in blacklist_patterns):
                continue

        # ✅ Извлекаем текст с проверкой минимальной длины
