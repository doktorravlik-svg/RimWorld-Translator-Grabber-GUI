...
def _xpath_matches_key(xpath: str, key: str) -> bool:
    ...
    ✅ ИСПРАВЛЕНО: Поддержка сложных XPath с предикатами типа [@Name='...'].
    Извлекает базовый путь и проверяет соответствие ключа.
    ...
    # Собираем возможные варианты соответствия
    # Ключ может выглядеть как: defName.field.subfield
    key_parts = key.split(".")

    # Проверяем последние части
    # XPath: Defs/ThingDef[@Name='Building'] -> parts = ['Defs', "ThingDef[@Name='Building']"]
    # Но после удаления предикатов: parts = ['Defs', 'ThingDef']

    # Проверяем соответствие последних частей пути
    # Сравниваем с конца ключа
    xpath_tags = [p.split("[")[0] for p in parts]  # Убираем предикаты если остались

    # Проверяем что все части XPath (без предикатов) есть в ключе в правильном порядке
    for i, xpath_part in enumerate(xpath_tags):
        if not xpath_part:
            continue
        # Ищем эту часть в ключе
        found = False
        for key_part in key_parts:
            if key_part.lower() == xpath_part.lower():
                found = True
                break
        if not found and xpath_part not in ("Defs", "defName"):
            # Проверяем вхождение (частичное совпадение)
            for key_part in key_parts:
                if xpath_part.lower() in key_part.lower() or key_part.lower() in xpath_part.lower():
                    found = True
                    break
        if not found:
            return False
