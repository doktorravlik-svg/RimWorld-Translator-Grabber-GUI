...
def display_language_table(languages, source_lang_info=None):
    ...
    """
    print("\n" + "=" * 70)
    print(f"{'Язык':<20} {'Keyed':<10} {'DefInjected':<12} {'Всего XML':<15} {'Статус':<10}")
    print("=" * 70)

    for lang_name, info in sorted(languages.items()):
        status = "✓ Готов" if info["total_xml_files"] > 0 else "✗ Пусто"
        if source_lang_info and source_lang_info.get(lang_name):
            src_total = source_lang_info[lang_name]["total_xml_files"]
            if src_total > 0:
                status = f"{(info['total_xml_files'] / src_total * 100):.1f}%"
        print(
            f"{lang_name:<20} {info['keyed_files']:<10} {info['def_files']:<12} {info['total_xml_files']:<15} {status:<10}"
        )
    print("=" * 70)
