...
class ModTranslationInfo:
    ...
    status_text: str  # Текст для отобраки (✅ Переведён, ⚠️ Частично и т.д.)
    status_icon: str  # Иконка для отобраки
...
class ModTranslationChecker:
    ...
    def _check_embedded_translation(self, mod_path: str) -> ModTranslationInfo:
        ...
        # Ищем Languages/Russian
        langs_path = os.path.join(mod_path, "Languages")
        if os.path.exists(langs_path):
            ru_path = os.path.join(langs_path, "Russian")
            if os.path.exists(ru_path):
                russian_path = ru_path
        ...
        has_keyed = os.path.exists(os.path.join(russian_path, "Keyed"))
        has_def_injected = os.path.exists(os.path.join(russian_path, "DefInjected"))
        ...
        if has_def_injected or has_keyed:
            ...
            if has_def_injected:
                for root, dirs, files in os.walk(os.path.join(russian_path, "DefInjected")):
                    translation_files += len([f for f in files if f.endswith(".xml")])

            if has_keyed:
                for root, dirs, files in os.walk(os.path.join(russian_path, "Keyed")):
                    translation_files += len([f for f in files if f.endswith(".xml")])
