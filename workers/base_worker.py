...
class BaseWorker(ABC):
    ...
    def stop(self, timeout: float = 5.0) -> None:
        ...
        # ✅ ИСПРАВЛЕНО: Не сбрасываем _is_running здесь
        # Это делает только _thread_worker в блоке finally
        # Это единственный источник истины о завершении потока

    def cancel(self, timeout: float = 5.0) -> None:
        """
        Отменить выполнение worker-а.

        Это псевдоним метода stop() с более понятным именем для GUI.
        Устанавливает флаг остановки и ждет завершения потока.
