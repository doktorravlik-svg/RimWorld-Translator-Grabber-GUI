...
class AutoTranslator:
    ...
    Args:
        ...
        config: Конфигурация (словарь с auto_split_glossary)
    """
    ...
    def __init__(
        ...
        self.source_lang: str = self._get_lang_code(source_lang)
        ...
        self.glossary_manager: GlossaryManager | None = glossary_manager
        auto_split = config.get("auto_split_glossary", True) if config else True
        ...
        self.translation_db = get_translation_db(self.target_lang) if HAS_TRANSLATION_DB else None

        if glossary_manager is None:
            if glossary_path:
                if os.path.isdir(glossary_path):
                    self.glossary_manager = GlossaryManager(
                        glossary_dir=glossary_path,
                        logger=logger,
                        use_morphy=False,
                        target_lang=target_lang,
                        auto_split_glossary=auto_split,
                    )
                else:
        ...
        if self.translation_db and self.glossary_manager:
            self.glossary_manager.import_from_db(self.translation_db)
            if self.logger:
                stats = self.translation_db.get_stats()
                self.logger.info(
                    f"Глоссарий импортирован: {stats.get('glossary_terms', 0)} терминов из БД"
                )
        ...
        if self.enabled:
            ...
            if self.logger:
                self.logger.info(f"[AutoTranslator] Получены engine_names: {engine_names}")
            else:
                print(f"[AutoTranslator] Получены engine_names: {engine_names}")
    ...
    def preload_common_terms(self, max_terms: int = 500) -> int:
        """
        Предзагружает кэш переводами из глоссария.

        Полезно для предварительной загрузки часто используемых терминов
        перед началом перевода DefInjected файлов.

        Args:
            max_terms: Максимальное количество терминов для загрузки
        ...
        # Ограничиваем количество загружаемых терминов
        entries_to_load = entries[:max_terms]

        loaded = self.cache.preload(entries_to_load, self.target_lang)
        if self.logger:
            self.logger.info(
                f"Предзагружено {loaded} терминов из глоссария в кэш"
            )
        return loaded
    ...
    def translate(self, text: str, original_text: OptionalStr = None) -> TranslationResult:
        ...
        cached = self.cache.get(text, self.target_lang, "auto")
        if cached:
            if self.logger:
        ...
        # ✅ ИСПРАВЛЕНО: Глоссарий применяем ПОСЛЕ API-перевода, а не до
        # Это предотвращает частичные переводы, когда глоссарий заменяет отдельные слова
        # До перевода, а потом API видит смешанный текст и не переводит остаток
        try:
            # Умное разбиение длинного текста
            if self.split_long_text and len(text) > self.max_chunk_size:
            ...
            if translated:
                # ✅ ИСПРАВЛЕНО: Предотвращение бесконечного автоперода
                ...
                if self.glossary_manager:
                    translated = self.glossary_manager.apply_to_text(translated)
                elif self.glossary:
                    translated = self.glossary.apply_to_text(translated)

                # Морфологическое склонение для русского/украинского
                # Передаем: русский текст, перевод из глоссария, оригинальное слово
                glossary_for_morphy = self.glossary_manager if self.glossary_manager else self.glossary
                if hasattr(glossary_for_morphy, '_use_morphy') and glossary_for_morphy._use_morphy:
                    translated = glossary_for_morphy.apply_morphology(translated, translated, text)

                # Сохраняем в кэш
                self.cache.set(text, self.target_lang, translated, "auto")
                ...
                # Применяем авто-коррекцию (исправление смешивания алфавитов, суржика)
                translated = self.language_rules.auto_correct(translated)

                # Проверяем, что перевод допустим (не частичный)
                if not self.language_rules.is_valid_translation(translated):
                    if self.logger:
                        self.logger.warning(
                            f"Partial translation detected, skipping: '{text[:LOG_PREVIEW_LENGTH]}...' -> '{translated[:LOG_PREVIEW_LENGTH]}...'"
                        )
                    return None
            ...
            if self._logger:
                self._logger.exception("Ошибка при переводе")
            return None
    ...
    def translate_batch(self, texts: TextList) -> TextList:
        ...
        for text in texts:
            ...
            # Переводим без глоссария (глоссарий применится ПОСЛЕ)
            translated = self.fallback_chain.translate(text)

            if translated:
                # ✅ Глоссарий применяем ПОСЛЕ API-перевода
                if self.glossary_manager:
                    translated = self.glossary_manager.apply_to_text(translated)
                elif self.glossary:
                    translated = self.glossary.apply_to_text(translated)
                ...
                if not self.language_rules.is_valid_translation(translated):
                    if self.logger:
                        self.logger.warning(
                            f"Partial translation detected in batch, skipping: '{text[:30]}...' -> '{translated[:30]}...'"
                        )
