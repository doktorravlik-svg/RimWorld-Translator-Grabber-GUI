...
class FallbackChain:
    ...
    def __init__(
        ...
        # ✅ ОТЛАДКА: Показываем какие движки реально создаGаны
        created_names = [type(engine).__name__ for engine in self.engines]
        if self.logger:
            self.logger.info(f"[FallbackChain] СоздаGаны движки: {created_names}")
        else:
            print(f"[FallbackChain] СоздаGаны движки: {created_names}")
    ...
    def _create_engine(self, name: str) -> TranslationEngine | None:
        ...
        try:
            if name == "google":
                ...
                return GoogleEngine(source_lang=self.source_lang, target_lang=self.target_lang, proxy=self.proxy)
            elif name == "deepl":
                from translation.engines.deepl_engine import DeepLEngine
                
                return DeepLEngine(source_lang=self.source_lang, target_lang=self.target_lang, proxy=self.proxy)
            elif name == "mymemory":
                from translation.engines.mymemory_engine import MyMemoryEngine
                
                return MyMemoryEngine(source_lang=self.source_lang, target_lang=self.target_lang, proxy=self.proxy)
            elif name == "bing":
                from translation.engines.bing_engine import BingEngine
                
                return BingEngine(source_lang=self.source_lang, target_lang=self.target_lang, proxy=self.proxy)
            elif name == "libre":
                from translation.engines.libre_engine import LibreEngine
                
                return LibreEngine(source_lang=self.source_lang, target_lang=self.target_lang, proxy=self.proxy)
            elif name == "argos":
                from translation.engines.argos_engine import ArgosEngine
                
                return ArgosEngine(source_lang=self.source_lang, target_lang=self.target_lang, proxy=self.proxy)
            elif name == "deeplx":
                from translation.engines.deeplx_engine import DeepLXEngine
                
                return DeepLXEngine(source_lang=self.source_lang, target_lang=self.target_lang, proxy=self.proxy)
            elif name == "translators":
                from translation.engines.translators_engine import TranslatorsEngine
                
                return TranslatorsEngine(source_lang=self.source_lang, target_lang=self.target_lang, proxy=self.proxy)
