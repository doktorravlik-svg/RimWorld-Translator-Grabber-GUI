...
class TranslationCache:
    ...
    def _make_key(self, text: str, target_lang: str, source_lang: str = "") -> str:
        ...
        # Для длинных текстов используем хэш (SHA-256 для безопасности)
        if len(raw_key) > 200:
    ...
    def preload(
        ...
        """
        Предзагружает кэш переводами из списка пар (оригинал, перевод, источник).

        Полезно для предварительной загрузки часто используемых терминов из глоссария.

        Args:
            entries: Список кортежей (original, translated, source)
            target_lang: Целевой язык
        ...
        loaded = 0
        with self._lock:
            for original, translated, _source in entries:
                if not original or not translated:
                    continue
                key = self._make_key(original, target_lang, "")
