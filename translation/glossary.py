...
class Glossary:
    ...
    def apply_morphology(self, translated_text: str, translation: str, original_text: str) -> str:
        """
        Применяет морфологическое склонение к переводу.

        Для русского/украинского языка возвращает морфологичный перевод
        с учетом морфологической формы в исходном тексте.

        Логика:
        1. Получаем морфологические формы перевода (на целевом языке)
        2. Находим, какая форма встречается в translated_text
        3. Возвращаем эту форму

        Args:
            translated_text: Русский текст после перевода (ищем в нем формы)
            translation: Перевод (базовая форма из глоссария)
            original_text: Оригинальный текст на исходном языке (для отладки)
        ...
        # translated_text is the Russian text, we need to find
        # which morphological form of the translation appears in it
        try:
            # Get all forms of the translation (Russian word)
            forms = self._morph_parser.get_word_forms(translation)
            if forms:
                # Check each form against the translated text
                for form_name, form_word in forms.items():
                    if form_word and form_word.lower() in translated_text.lower():
                        return form_word
        except Exception as e:
            if self.logger:
                self.logger.debug(f"Morphology error for '{translation}': {e}")
