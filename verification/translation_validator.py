...
class TranslationValidator:
    ...
    def __init__(self, language: str = "ru", logger: Any | None = None):
        ...
        try:
            ...
            from config.language_constants import get_lang_code_from_name

            target_lang = get_lang_code_from_name(language)
            self.translation_db = get_translation_db(target_lang)
    ...
    def validate(self, key: str, original: str, translated: str) -> ValidationResult:
        ...
        # ✅ НОВОЕ: 6. Проверка на смешанный язык (двухязычный контент)
        mixed_lang_issues = self._validate_mixed_language(original, translated)
        issues.extend(mixed_lang_issues)

        # ✅ НОВОЕ: 7. Проверка терминологии по глоссарию
    ...
    def _validate_mixed_language(self, original: str, translated: str) -> list[ValidationIssue]:
        """
        Проверяет наличие смешанного языка (двухязычного контента).
        ...
        Returns:
            Список проблем с смешанным языком
        """
        issues = []
        ...
        try:
            ...
            has_cyrillic = bool(cyrillic_pattern.search(translated))
            has_latin = bool(latin_pattern.search(translated))

            if has_cyrillic and has_latin:
                latin_words = latin_pattern.findall(translated)
                if len(latin_words) >= 3:
                    issues.append(
                        ValidationIssue(
                            severity=ValidationSeverity.WARNING,
                            code="MIXED_LANGUAGE_CONTENT",
                            message=f"Обнаружен смешанный язык: найдены английские слова '{', '.join(latin_words[:3])}' рядом с кириллицей",
                            context=f"Перевод содержит {len(latin_words)} английских слов",
                        )
                    )
        except Exception as e:
            if self.logger:
                self.logger.debug(f"Ошибка проверки смешанного языка: {e}")
