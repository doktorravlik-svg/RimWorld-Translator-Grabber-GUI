...
class LLMDetector:
    ...
    def __init__(self):
        ...
        self.BRACKET_CLEAN_PATTERN = re.compile(r'\[.*?\]')
        ...
        self.case_hallucinations = [
            ...
            (re.compile(r"при [а-я]{3,}ом\b", re.IGNORECASE), "Творительный падеж вместо предложного: 'при [чем-то]ом'"),
            # Лишний предлог или его отсутствие в сложных конструкциях
            (re.compile(r"с целью для того чтобы\b", re.IGNORECASE), "Избыточный перевод с агл. to: 'с целью для того чтобы'"),
            # Характерная агл./LLM ошибка: it is -> это есть
            (re.compile(r"это есть (\w+[аяиеё])\b", re.IGNORECASE), "LLM-перевод конструкции 'it is': используйте просто 'это' или 'это [прилагательное]'"),
            # Прямолинейный перевод there is/are
            (re.compile(r"там (является|есть) [а-я]+", re.IGNORECASE), "Прямолинейный перевод there is/are: напишите естественно"),
            # Избыточное использование пассивного залога (характерно для нейросетей)
            (re.compile(r"(был|была|было|были) (\w+ен|\w+ена|\w+ено|\w+ены) (кем-то|кем-либо)", re.IGNORECASE),
             "Канцелярит/LLM-перевод: пассивный залог + 'кем-то' — перефразируйте"),
            # Характерная ошибка: of = про/о (сверхбуквальный перевод)
            (re.compile(r"о [а-я]{3,}е \b(\w{4,})\b", re.IGNORECASE), "Возможно, буквальный перевод of: проверьте контекст"),
            # Страдание от конструкции «make + do»
            (re.compile(r"сделать (кто-то) (\w+ым|\w+ой)", re.IGNORECASE), "LLM-перевод make+прил: лучше 'заставить кого-то сделать что-то'"),
            # Страдательный залог там, где нужен активный
            (re.compile(r"бывает? (\w+ен|\w+ена|\w+ено)", re.IGNORECASE), "Пассивный залог в описании обыденного: перефразируйте"),
        ]
    ...
    def _check_rulepack_llm_artifacts(self, text: str) -> list[dict]:
        ...
        # Безопасный паттерн: ищем <li> с содержимым до </li> или конца строки
        li_contents = re.findall(r"<li>(.*?)\)?->(.*?)(?:</li>|$)", text, re.DOTALL)
        for full, result in li_contents:
            clean_result = self.BRACKET_CLEAN_PATTERN.sub('', result).strip()
