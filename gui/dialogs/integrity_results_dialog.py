...
class IntegrityResultsDialog:
    ...
    def __init__(self, parent, result, log_messages=None):
        ...
        # ✅ ИСПРАВЛЕНО: Очистка данных при закрытии диалога
        self.dialog.bind("<<Destroy>>", self._on_destroy)

        # ✅ ИСПРАВЛЕНО: Очистка данных при закрытии диалога
        self.dialog.bind("<<Destroy>>", self._on_destroy)

        # ✅ ИСПРАВЛЕНО: Центрирование относительно родителя
    ...
    def _build_ui(self):
        """Построить интерфейс"""
        # ── Сводка ──
        summary_frame = ttk.LabelFrame(self.dialog, text=tr("integrity_summary", "📊 Сводка"))
        ...
        # ── Дерево результатов ──
        tree_frame = ttk.LabelFrame(self.dialog, text=tr("integrity_details", "Детали"))
        tree_frame.pack(fill="both", expand=True, padx=10, pady=5)

        columns = ("type", "file", "description")
        self.tree = ttk.Treeview(tree_frame, columns=columns, show="headings", height=15)

        self.tree.heading("type", text=tr("deps_type", "Тип"))
        self.tree.heading("file", text=tr("editor_file", "Файл"))
        self.tree.heading("description", text=tr("glossary_description", "Описание"))
        ...
        ttk.Button(
            btn_frame, text=tr("integrity_copy_report", "📋 Копировать отчёт"), command=self._copy_report, bootstyle="info"
        ).pack(side="left", padx=2)

        ttk.Button(
            btn_frame, text=tr("integrity_export_file", " Экспорт в файл"), command=self._export_report, bootstyle="secondary"
        ).pack(side="left", padx=2)

        ttk.Button(
            btn_frame, text=tr("editor_open_folder", "📂 Открыть папку"), command=self._open_folder, bootstyle="info"
        ).pack(side="left", padx=2)

        ttk.Separator(btn_frame, orient="vertical").pack(side="left", padx=10, fill="y")

        ttk.Button(
            btn_frame, text=tr("editor_close", "✖ Закрыть"), command=self.dialog.destroy, bootstyle="danger"
    ...
    def _open_folder(self):
        ...
        messagebox.showinfo(tr("verification_no_files", "Информация"), tr("verification_no_files_available", "Нет доступных файлов для открытия"))
    ...
    def _copy_report(self):
        ...
        messagebox.showinfo(tr("verification_copied", "Скопировано"), tr("verification_report_copied", "Отчёт скопирован в буфер обмена"))
        ...
        if file_path:
            try:
                ...
                with open(file_path, "w", encoding="utf-8") as f:
                    f.write(report)
                messagebox.showinfo(tr("editor_success", "Готово"), tr("verification_report_saved", f"Отчёт сохранён:\n{file_path}"))
            except Exception as e:
                messagebox.showerror(tr("glossary_error", "Ошибка"), tr("verification_save_failed", f"Не удалось сохранить отчёт:\n{e}"))
    ...
    # ✅ ИСПРАВЛЕНО: Очистка данных при уничтожении диалога
    def _on_destroy(self, event=None):
        """Очищает словари и ссылки для предотвращения утечек памяти"""
