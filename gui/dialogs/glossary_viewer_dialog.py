...
class GlossaryViewerDialog:
    ...
    def _create_dialog(self):
        ...
        self.category_combo.bind("<<ComboboxSelected>>", lambda e: self._load_glossary(self.search_var.get().lower()))
    ...
    def _load_glossary(self, filter_text=""):
        ...
        items = self.db.search_glossary(filter_text, self.target_language) if filter_text else self.db.get_all_glossary(target_language=self.target_language)
        for t in items:
            if hasattr(t, "keys"):
                category = t["category"]
