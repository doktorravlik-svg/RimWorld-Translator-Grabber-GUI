...
class FileHistoryDialog:
    ...
    def __init__(self, parent, file_path, editor=None, target_language=None):
        """
        Args:
            parent: Родительское окно
            file_path: Путь к файлу
            editor: Ссылка на TranslationEditorDialog для восстановления версий
            target_language: Язык для базы данных
        """
        self.parent = parent
    ...
    def _create_dialog(self):
        ...
        ttk.Button(bf, text=tr("history_save_version", " Сохранить версию"), command=self._save_version).pack(
            side="left", padx=2
        )
        ttk.Button(bf, text=tr("history_restore_version", "↩️ Восстановить"), command=self._restore_version).pack(
            side="left", padx=2
        )
        ttk.Button(
    ...
    def _restore_version(self):
        ...
        if messagebox.askyesno(
            ...
            # ✅ ИСПРАВЛЕНО: Сначала загружаем старую версию
            data = self.db.get_file_version(self.file_path, ver)
            
            if data:
                # Если загрузка успешна — сохраняем текущую версию как бэкап
                ...
                messagebox.showerror(
                    ...
                    tr("editor_version_load_failed", "Не удалось загрузить версию {ver}").format(ver=ver),
                )
