...
class LanguageSelector(ttk.Frame):
    ...
    def __init__(
        ...
        if command:
            self.language_var.trace_add("write", self._on_change)

    def _on_change(self, *args):
        if self._command:
            self._command(self.language_var.get())
...
class MultiLanguageSelector(ttk.Frame):
    """Компонент выбора нескольких языков с поддержкой добавления/удаления"""

    def __init__(
        self,
        parent,
        label_text=tr("source_languages", "Исходные языки:"),
        languages=None,
        initial_values=None,
        command=None,
    ):
        ...
        self.add_button = ttk.Button(self, text="+", width=3, command=self._add_language)
        self.add_button.pack(side="right", padx=(5, 0))

        self._init_language_row()

        if initial_values:
            for lang in initial_values:
                if lang not in [v.get() for v in self.language_vars]:
                    self._add_language(lang)

    def _init_language_row(self, lang="English"):
        var = tk.StringVar(value=lang)
        self.language_vars.append(var)

        frame = ttk.Frame(self.container)
        frame.pack(fill="x", pady=2)

        combo = ttk.Combobox(frame, textvariable=var, values=self.languages, width=18)
        combo.pack(side="left", padx=(0, 5))

        remove_btn = ttk.Button(frame, text="×", width=3, command=lambda: self._remove_language(frame, var))
        remove_btn.pack(side="right")

        self.language_entries.append((frame, combo))

        var.trace_add("write", self._on_change)

    def _add_language(self, lang=None):
        if lang is None:
            lang = "English"
        
        existing = [v.get() for v in self.language_vars]
        available = [l for l in self.languages if l not in existing]
        if available:
            lang = available[0]
        
        self._init_language_row(lang)
        self._on_change()

    def _remove_language(self, frame, var):
        if len(self.language_vars) <= 1:
            return
        
        frame.destroy()
        self.language_vars.remove(var)
        self.language_entries = [(f, c) for f, c in self.language_entries if f != frame]
        self._on_change()
    ...
    def get(self):
        return [v.get() for v in self.language_vars if v.get()]

    def set(self, values):
        for frame, combo in self.language_entries:
            frame.destroy()
...
class LogPanel(ttk.LabelFrame):
    ...
    def __init__(self, parent, title=None, height=10):
        ...
        self.log_text.tag_config("info", foreground="#1976D2")  # Синий (Material Blue)
        self.log_text.tag_config("warning", foreground="#F57C00")  # Оранжевый
        self.log_text.tag_config("error", foreground="#D32F2F")  # Красный
        self.log_text.tag_config("success", foreground="#388E3C")  # Зеленый
        self.log_text.tag_config("debug", foreground="#757575")  # Серый
    ...
    def _determine_level(self, message):
        """Определить уровень сообщения с поддержкой формата loguru"""
        msg = message.strip()
        msg_lower = msg.lower()

        # 1. Попытка определить по формату loguru: "2026-05-04 14:10:24 | LEVEL     | ..."
        if " | INFO     |" in msg or msg.startswith("ℹ") or "info" in msg_lower or "инфо" in msg_lower:
            return "info"
        elif " | WARNING  |" in msg or msg.startswith("⚠️") or "warning" in msg_lower or "предупреждение" in msg_lower:
            return "warning"
        elif " | ERROR    |" in msg or msg.startswith("❌") or "error" in msg_lower or "ошибка" in msg_lower:
            return "error"
        elif " | DEBUG    |" in msg or msg.startswith("🔍") or "debug" in msg_lower or "отладка" in msg_lower:
            return "debug"
        elif " | SUCCESS  |" in msg or msg.startswith("✅") or "success" in msg_lower or "успех" in msg_lower:
            return "success"

        # 2. Проверка по старым эмодзи (если loguru не используется)
        if "ошибка" in msg_lower or "error" in msg_lower:
            return "error"
        elif "предупреждение" in msg_lower or "warning" in msg_lower:
            return "warning"
        elif "успех" in msg_lower or "success" in msg_lower:
            return "success"
