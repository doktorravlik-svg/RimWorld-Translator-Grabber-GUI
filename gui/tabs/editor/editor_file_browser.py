...
class TranslationEditorTab(ttk.Frame):
    ...
    def _show_context_menu(self, event):
        ...
        if item:
            ...
            m = ttk.Menu(self, tearoff=0)
            m.add_command(
                label=tr("ctx_open_in_editor", "✏️ Open in Editor"),
                command=self._open_selected_file,
            )
            m.add_separator()
            m.add_command(
                label=tr("ctx_copy_path", "📋 Copy Path"),
                command=self._copy_selected_path,
            )
            m.add_command(
                label=tr("ctx_open_folder", "📂 Open Folder"),
                command=self._open_selected_folder,
            )
            m.add_separator()
            m.add_command(
                label=tr("ctx_file_info", "📊 File Info"),
                command=self._show_file_info
            )
            m.add_separator()
            m.add_command(
                label=tr("ctx_refresh_list", "🔄 Refresh List"),
                command=self._refresh,
            )
