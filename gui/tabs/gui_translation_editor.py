...
class TranslationEditorDialog:
    ...
    def __init__(self, parent, file_path: str = "", title: str = "Редактор перевода"):
        ...
        self.modified = False
        self.entries = []
        self.file_map = {}  # Для режима папки: key -> имя файла
        self.folder_mode = False  # ✅ Флаг режима папки
    ...
    def _undo(self):
        if self.history_manager.undo_last_edit():
            self._update_undo_redo_buttons()
            self.modified = True

    def _redo(self):
        if self.history_manager.redo_last_edit():
            self._update_undo_redo_buttons()
            self.modified = True
    ...
    def _translate_empty(self):
        ...
        if ec == 0:
            ...
            return
        if messagebox.askyesno(
            "Подтверждение", f"Найдено {ec} пустых записей. Создать базовый перевод?"
        ):
            cnt = 0
            for e in self.entries:
            ...
            messagebox.showinfo(
                self.tr("editor_success", "Успех"),
                self.tr("editor_translated_empty", "Переведено {cnt} пустых записей").format(
                    cnt=cnt
                ),
            )
    ...
    def _show_glossary(self):
        ...
        GlossaryViewerDialog(self.dialog, editor=self)
    ...
    def _show_file_history(self):
        ...
        FileHistoryDialog(self.dialog, self.file_path, editor=self)

    def _show_suggestions(self):
        """Показать подсказки перевода (вынесено в отдельный диалог)"""
        from gui.dialogs.suggestions_dialog import SuggestionsDialog

        SuggestionsDialog(self.dialog, self.entries, self.file_path, editor=self)


# ============================================================
# Перенесено в gui/tabs/editor/:
#   WrappingToolbar      → editor_toolbar.py
#   TranslationEditorTab → editor_file_browser.py
# Импортируйте для обратной совместимости:
#   from gui.tabs.editor import WrappingToolbar, TranslationEditorTab
# ============================================================
