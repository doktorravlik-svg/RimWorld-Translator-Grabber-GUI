...
class DuplicatesTab(ttk.Frame):
        ...
        try:
            ...
            if os.path.exists(os.path.join(mods_folder, "About")) or os.path.exists(
                ...
                try:
                    ...
                    self.after(0, lambda: self.log(f"⚠️ Ошибка чтения папки модов: {e}"))
    ...
    def _on_merge(self):
        ...
        self.progress_bar.configure(mode="determinate")
        self.progress_label.config(text="🔄 Слияние дубликатов...")
    ...
    def finish_progress(self, success=True):
        ...
        if success:
            ...
            self.progress_label.config(text="✅ Слияние завершено успешно")
        else:
            self.progress_bar.configure(bootstyle="danger-striped")
            self.progress_label.config(text="❌ Ошибка при слиянии")
