...
class SettingsFontManager:
    ...
    def create_fonts_section(self, container: ttk.Frame) -> ttk.Frame:
        ...
        """
        section = CollapsingFrame(container, text="🔤 Шрифты", collapsed=False)
        ...
        for key, label, preview_text in fonts:
            self._create_font_row(frame, key, label, preview_text)
    ...
    def _apply_main_font_to_all(self) -> None:
        ...
        main_font = self.config.get("main_font", {})
    ...
    def _reset_all_fonts(self) -> None:
        ...
        if messagebox.askyesno(tr("glossary_reset_fonts_confirm", "Сброс шрифтов"), tr("glossary_reset_fonts_confirm", "Сбросить все шрифты к значениям по умолчанию?")):
